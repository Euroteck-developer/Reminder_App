...
const updateMeetingStatus = (req, res) => {
  ...
  db.query("SELECT created_by FROM meetings WHERE id = ?", [meetingId], (err, rows) => {
    if (err) return res.status(500).json({ error: err.message });
    if (!rows.length) return res.status(404).json({ message: "Meeting not found" });

    if (Number(rows[0].created_by) !== Number(userId)) {
      return res.status(403).json({ message: "Only the creator can update the meeting status" });
    }

    db.query("UPDATE meetings SET status = ? WHERE id = ?", [status, meetingId], (err2) => {
      if (err2) return res.status(500).json({ error: err2.message });
      res.json({ message: `Meeting status updated to '${status}'` });
    });
  });
...
const deleteMeeting = (req, res) => {
  ...
  db.query("SELECT created_by FROM meetings WHERE id = ?", [meetingId], (err, result) => {
    if (err) return res.status(500).json({ error: err.message });
    if (!result.length) return res.status(404).json({ message: "Meeting not found" });
    ...
    db.query("DELETE FROM meeting_assignees WHERE meeting_id = ?", [meetingId]);
    db.query("DELETE FROM meeting_departments WHERE meeting_id = ?", [meetingId]);
    db.query("DELETE FROM notifications WHERE message LIKE ?", [`%Meeting ID: ${meetingId}%`]);

    db.query("DELETE FROM meetings WHERE id = ?", [meetingId], (err2, result2) => {
      if (err2) return res.status(500).json({ error: err2.message });
