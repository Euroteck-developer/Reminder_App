...
const markReminderSent = (reminderId) => {
  db.query("UPDATE task_self_reminders SET sent = 1 WHERE id = ?", [reminderId], (err) => {
    if (err) console.error("Cron Update Error:", err);
  });
};
...
cron.schedule("* * * * *", () => {
  const query = `
    SELECT 
      r.id, r.task_id, r.reminder_datetime,
...
module.exports = { getSelfReminder, saveSelfReminder, markReminderSent };
