...
const sendEmail = async (to, subject, html) => {
  return sgMail.send({
    ...
    from: {
      name: "Reminder App",
      email: process.env.EMAIL_FROM,  // Must match verified sender in SendGrid
    },
    subject,
    html,
  });
...
// Convert JS datetime → MySQL datetime
const toMySQLDate = (date) => {
  const d = new Date(date);
  const pad = (n) => (n < 10 ? "0" + n : n);
